...
public class BlockPotionFlask : BlockLiquidContainerTopOpened
{
                ...
                (byEntity as EntityPlayer)?.WalkInventory(
                    pslot =>
                    {
                        if (
                            ...
                            return true;
                        var mergableq = slot.Itemstack.Collectible.GetMergableQuantity(
                            slot.Itemstack,
                            pslot.Itemstack,
    ...
    )
    {
        ...
        var content = GetContent(inSlot.Itemstack);
    ...
    )
    {
        ...
        if (contentStack != null)
        {
            ...
            var contentSource = new FlaskTextureSource(
                capi,
                contentStack,
                props.Texture,
                this
    ...
    )
    {
        ...
        {
            var meshdata = GenMesh(capi, contentStack);
            if (meshdata == null)
                return;
    ...
    )
    {
        ...
        try
        {
            ...
            if (effects?.Exists ?? false)
            {
                ...
                {
                    case "strong":
                        foreach (var k in EffectDictionary.Keys.ToList()) EffectDictionary[k] *= 4;

                        break;
                    case "medium":
                        foreach (var k in EffectDictionary.Keys.ToList()) EffectDictionary[k] *= 2;
        ...
        byEntity.World.RegisterCallback(
            dt => playEatSound(byEntity, "drink"),
            500
        );
    ...
    {
        var pos = byEntity.Pos.AheadCopy(0.4f).XYZ.Add(byEntity.LocalEyePos);
        pos.Y -= 0.4f;
    ...
    )
    {
        ...
        {
            var tickPotion = content.ItemAttributes?["tickpotioninfo"];
            bool effectSuccessfullyApplied;
            if (tickPotion?.Exists ?? false)
            {
                var potionEffect = new TempEffect();
        ...
        switch (potionId)
        {
                    ...
                    playerEntity.World.RegisterCallback(
                        dt =>
                        {
                            ...
                            if (playerEntity.WatchedAttributes.GetLong("recallpotionid") != 0)
                            {
                                ...
                                var spawn = serverPlayer.GetSpawnPosition(false);
            ...
            {
                var hungerTree = playerEntity.WatchedAttributes.GetTreeAttribute(
                    "hunger"
                );
                if (hungerTree != null)
                {
                    var maxSaturation = hungerTree.GetFloat("maxsaturation");
                    var fraction = Math.Max(maxSaturation / 20f, 0);

                    hungerTree.SetFloat("currentsaturation",
                        Math.Min(hungerTree.GetFloat("currentsaturation") + fraction, maxSaturation));
    ...
    public TextureAtlasPosition this[string textureCode]
    {
        get
        {
            ...
            if (contentTextPos == null)
            {
                ...
                textureSubId = ObjectCacheUtil.GetOrCreate(
                    capi,
